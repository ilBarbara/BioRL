...
double getWeight_e(int *set, int t_i, int *ks, int *gi_sum, int** kelem, int n, mutation_data_t *A, int use_marginHash){
  // Declarations
  geneset_t *key;
  weight_t *w;
  int i, j;
  ...
  if (w == NULL) {
    ...
    double score;
    int func;
    comet_phi_e(key->genes, k, n, A, ctbl, kelem, &score, &func, co_cutoff, permutation_iteration, binom_pvalthreshold, pvalthresh, use_marginHash);
    returnW = -log(pow(score, mcmc_amp));
    w->weight = returnW;
    w->function = func;
    if (func == 1 || func == 3){ // store weight only if from permutation test and exact test with enumerating table > x      
...
void rand_soln(int *set, double *weights, int m, int n, int t, int* gi_sum, int* ks, int** kelem, int num_genes, mutation_data_t *A, int use_marginHash){
  // Declarations
  int i, r, j=0;
  ...
  for(i = 0; i < num_genes; i++){
    ...
    if (i == gi_sum[j]-1){      
      weights[j] = getWeight_e(set, j, ks, gi_sum, kelem, n, A, use_marginHash);      
      j++;
    }    
  }  
}
...
// Multi-Dendrix MCMC
double comet_mcmc(mutation_data_t *A, int m, int n, int *ks, int t, int num_iters, int step_len, int amp, 
                    ...
                    int verbose, int use_marginHash){
  lookups = 0;
  insertToHash = 0;
  calculations = 0;
  printf("  - Initial hashed gene sets: %d\n", HASH_COUNT(weightHash));
  if (use_marginHash) {
    m2w_info = malloc(sizeof(int) * 3);
    memset(m2w_info, 0, sizeof(int) * 3); // 0:lookups, 1:insertToHash = 0, 2:calculations = 0
    printf("  - Initial hashed margin keys: %d\n", HASH_COUNT(margin2weightHash));
  }
  // Declarations
  ...
  if (size_initial_soln == 0){    
    rand_soln(set, weights, m, n, t, group_index_sum, ks, k_elem, num_genes, A, use_marginHash);    
  }
  else{
  ...
  double max_w = current_weight;
  int max_index = num_iters / step_len - 1;
  printf("max_index: %d \n", max_index);
  for (j=0; j < num_genes; j++) gene_sets[max_index][j] = set[j];
  for (j=0; j < t; j++) {
    set_weights[max_index][j] = weights[j];        
    set_functions[max_index][j] = getFunction(set, j, ks, group_index_sum);
  }
  ...
  while (1) {
    if (insertToHash >= num_iters) {
      break;
    }
    // printf("#iter %d, current_weight: %f \n", i, current_weight);
    while(1){
      // next_gene        = (double) (rand() % m); // sampling a new gene
      next_gene        = rand() % m; // sampling a new gene
    ...
    next_weights[set_to_exchange] = getWeight_e(set, set_to_exchange, ks, group_index_sum, k_elem, n, A, use_marginHash);    
    ...
    if (dendrix_pass && gene_index != -1){ // swapped, need to update another set with exchanged gene
      ...
      if (set_to_exchange != other_set_to_exchange){
        next_weights[other_set_to_exchange] = getWeight_e(set, other_set_to_exchange, ks, group_index_sum, k_elem, n, A, use_marginHash);
      }
    }
    ...
    if ( ((i+1) % step_len == 0) && (step_index < max_index) ){
  ...
  if (verbose == 1){
    ...
    if (use_marginHash) {
      printf("  - m2w Look ups: %d\n", m2w_info[0]);
      printf("  - m2w Unique margin keys: %d\n", m2w_info[2]);
      printf("  - m2w Hashed margin keys: %d\n", m2w_info[1]);
    }
    end = clock();
