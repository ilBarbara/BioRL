...
int exact_test_helper(double *pval, int *num_tbls, int k, double pvalthresh, int num_entries,
                      int N, double numerator, int *margins, int *ex_cells, int *co_cells,
                      int num_co_cells, int *tbl, int **mar_stack, int co_in, int T_rem, int T_obs){
  int res = UNDER_THRESH;    // UNDER_THRESH = 0
...
double comet_exact_test(int k, int N, int *ctbl, int *final_num_tbls, double pvalthresh, int use_marginHash){
  ...
  if (use_marginHash) {
    marginkey_t *markey;
    margin2weight_t *m2w;
    markey = malloc( sizeof(marginkey_t) );
    memset(markey, 0, sizeof(marginkey_t));
    for (i = 0; i < k; i++) markey->margin_key[i] = margins[i+k];
    qsort(markey->margin_key, k, sizeof(int), ascending);
    markey->margin_key[k] =  Tobs;
    
    HASH_FIND(hh, margin2weightHash, markey, sizeof(marginkey_t), m2w);  /* id already in the hash? */
    
    if (m2w == NULL) {
      m2w_info[2] += 1;
      m2w = malloc(sizeof(margin2weight_t));
      m2w->id = *markey; 
      ...
      *final_num_tbls = num_tbls[0];
      ...
      m2w->weight = (res == OVER_THRESH) ? res : final_p_value;
      m2w_info[1] += 1;
      HASH_ADD( hh, margin2weightHash, id, sizeof(marginkey_t), m2w );  /* id: name of key field */
      return m2w->weight;
    }
    else {
      m2w_info[0] += 1;
...
void comet_phi(int *genes, int k, int n, mutation_data_t *A, int *ctbl, int** kelem, double *score, int *func, int co_cutoff, int permutation_iteration, double binom_pvalthresh, double exact_pvalthresh){
  /*        [have read and understood]
    genes: genes in the set, for permutation test
  ...
  */  
  ...
  if (k > 3){ // heuristic pipeline when k > 3    
    ...
    if (conum  > co_cutoff || binom_pval > binom_pvalthresh){         
      ...
      exact_pval = comet_exact_test(k, n, ctbl, &final_num_tbls, exact_pvalthresh, 0);
      if (exact_pval == -1){ // stop when pval > 0.001, do permutation test
    ...
    *score = comet_exact_test(k, n, ctbl, &final_num_tbls, 1.1, 0);
    if (final_num_tbls > numTableCutoff){
      *func = 1;
    }
    else{
...
void comet_phi_e(int *genes, int k, int n, mutation_data_t *A, int *ctbl, int** kelem, double *score, int *func, int co_cutoff, int permutation_iteration, double binom_pvalthresh, double exact_pvalthresh, int use_marginHash){
  /*        [have read and understood]
    genes: genes in the set, for permutation test
    k: the number of genes
    n: the number of total samples
  ...
  *score = comet_exact_test(k, n, ctbl, &final_num_tbls, 1.1, use_marginHash);
  *func = 1;
  // end = clock();
  // printf("  - exact test runtime: %f secs\n", (double)(end - begin) / CLOCKS_PER_SEC);
}
