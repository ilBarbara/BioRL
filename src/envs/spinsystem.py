...
class SpinSystemFactory(object):
    ...
    def get(graph_generator=None,
        ...
        if type(graph_generator) == str:
            return SpinSystemPvalue(graph_generator,max_steps, minFreq,
...
class SpinSystemBase(ABC):
    ...
    def __init__(self,
        ...
        assert type(graph_generator) == str
        # print(minFreq)
        mutations = C.load_mutation_data(graph_generator, None, None, minFreq)
        m, n = mutations[0], mutations[1]
        self.gnum, self.pnum = m, n
        ...
        self.iPatientToGenes, self.iGeneToCases, self.geneToNumCases, self.geneToIndex, self.indexToGene = cMutations
        self.P2G_arr = np.zeros((n, m), dtype=np.bool_)
        self.G2P_arr = np.zeros((m, n), dtype=np.bool_)
        for g_i in range(m):
            for p_i in self.iGeneToCases[g_i]:
                self.P2G_arr[p_i][g_i] = True
                self.G2P_arr[g_i][p_i] = True

        self.n_spins = self.gnum  # Total number of spins in episode
        self.max_steps = max_steps  # Number of actions before reset

        self.reward_signal = reward_signal
        self.norm_rewards = norm_rewards

        self.n_actions = self.gnum
        ...
        self.matrix = self.G2P_arr.astype(np.float32)
        self.bias = None
    ...
    def get_observation(self):
        ...
        return np.hstack((state.T, self.matrix_obs))
    ...
    def _calculate_score_change(self, new_spins, matrix, action):
        ...
        elif self.optimisation_target == OptimisationTarget.PVALUE:
            delta_score = self._calculate_pvalue_change(new_spins, matrix, action)
        else:
            raise NotImplementedError
        return delta_score
    ...
    @abstractmethod
    def _calculate_pvalue_change(self, new_spins, matrix, action):
        raise NotImplementedError


class SpinSystemPvalue(SpinSystemBase):
    def _calculate_pvalue_change(self, new_spins, matrix, action):
        if np.sum(new_spins == -1) < 2:
            return 0.0
        M_matrix = matrix[new_spins == -1, :]
        x = np.sum(M_matrix, axis=1)
        t = np.sum(np.sum(M_matrix, axis=0) == 1)
        N = self.pnum
        p = [[float(x_i) / N] * N for x_i in x]
        p_value = saddlepoint(t, x, p)
        # print(p_value, type(p_value))
        if np.isnan(p_value):
            print('Warning: saddlepoint approximation produces nan!')
            return 0.0
        weight = -math.log(p_value)
        if weight < 0:
            weight = 0.0
        return weight - self.score
